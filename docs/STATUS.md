...
- DONE (**Arsenal section — the technique showcase (public-page content feature, S1 of 2), PR #218**):
  the first slice of a two-part public-page **content** feature (S2 — the Gauntlet fighter bios — is the
  parked next slice). A new static **"The Arsenal"** section in `web/`, woven into the page arc **between
  How it works and the CTA**, showing all **13 karate techniques** grouped into 5 families (Strikes ·
  Kicks · Close-range · Takedowns · Aerial); each card carries its romaji move-id, English gloss, a
  one-line descriptor, and a **score badge** — the numeric WKF encoding (`1`/`2`/`3`/`2·3`/`0→3`) with a
  `role="img"` accessible label so `2·3` is announced as "scores 2 points, 3 to the head" rather than
  voiced literally. Plus an **"Arsenal"** nav anchor after "How it works" and a **single** end-of-section
  **`/spec`** frame-table hand-off (decorative `↗` aria-hidden; no per-card links). **Presentation-only** —
  the move roster is a hand-curated `readonly` array (kept in sync by hand with `docs/move-roster.md` +
  `CANONICAL_RULES`, **no `src/engine` import** — the web layer stays decoupled from the TCB); **no
  `INPUT_HASH` / `BENCHMARK_VERSION` ("v19") / TCB change**. Built in **5 ordered TDD increments**
...
9. **Move showcase + per-move poses — ✅ COMPLETE (all 9 stories S0–S8 shipped; PRs #350, #352,
   ...
   - **S4 ✅ (PRs #363–#367)** — `web/`-only, **six** slices (grown from four mid-flight), spending the
     mechanism on the two moves with real screen time: **`gyaku-zuki`** (~80%) and **`mawashi-geri`** (~13%).
     Slices 1–2 threw the reverse punch with the **rear hand** + a hip `hikite`, but the eye-check found the
     distinction faint — both arms hung off one shoulder, landing the extended arm the same place either way,
     which at the workhorse distance rendered a jab and a reverse punch on the **identical pixel**. That forced
     the **shoulder girdle** (M12): slice 3 derived the lean, slice 4 split the shoulder into two ends so the
     rear arm starts 14px back, and slice 5 made the girdle **rotate** — only the driving shoulder comes
     through — restoring `hikite` to the hip. Slice 6 hit the same wall on kicks as originally forecast
     (`mae-geri` and `mawashi-geri` both drive a foot to the same target, and a 2-D view cannot show hip
...
11. **Per-move character differentiation — arc in progress (S4 of 5, PRs #386, #388, #390, #392).** A follow-up to the
    ...
    **consciously amends M9** — "a kick is upright _by default_; an authored lean is the exception" (the
    `mae`/`mawashi` M9 tests keep their asserts, reworded premises). **S3 (#390)** — a new optional per-move
    **`arc`** descriptor (a via-waypoint the **wind-up** bows through) + a total `arcFor` lookup + a pure `bezier2`
    quadratic Bézier: `easeDriven`'s startup leg becomes `bezier2(stance, arc, chamber, u)` instead of a straight
    `lerpJoint`, so `mawashi-geri`'s rear foot **swings up-and-around** as it loads, reading apart from
    `ushiro-geri`'s straight rear-leg thrust. The Bézier **pins both endpoints** (`t=0`→stance, `t=1`→chamber), so the
