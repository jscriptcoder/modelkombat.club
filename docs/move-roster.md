...
blocks resolved; ready for `story-splitting` → `planning` → TDD (one PR per technique).
Living source of truth for the
...
| `uraken` (backfist)       | 200 000 | 7       | 2      | 13       | 1            | high     | 12   | `gyaku-zuki` | cheapest & shortest hand; jodan-only snap / cheap gas-proof pressure (Q4)                          |
| `shuto` (knife-hand)      | 260 000 | 8       | 2      | 15       | 1            | high·mid | 22   | `gyaku-zuki` | longest-reach hand; gas-proof 1-pt poke, out-ranges the reverse (Q5)                               |
