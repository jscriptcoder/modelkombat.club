...
**The entire variety arc (S1a–S5c) is now COMPLETE bar the no-build S5a**: the public
`/variety` web surface shipped too (S5c — PRs #295 page + #296 discoverability, plan
archived `docs/archive/variety-telemetry-s5c.md`). Only post-launch **S5a** (submission
corpus — no build) remains.
...
| **S5c** | "Can the community see the meta?" | **✅ SHIPPED + ARCHIVED** (decision #12, PRs #295 page + #296 discoverability; plan archived `docs/archive/variety-telemetry-s5c.md`): a static prerendered `/variety` page cloning `/spec-guide` — renders the `docs/variety.md` board via `marked`, `generateVariety()` regenerated at build, no client JS; discoverable via sitemap + llms.txt + an Arsenal link (NOT top-nav) | — | `/variety` serves the rendered board as no-JS HTML; sitemap + llms.txt list it | public web; **LIVE** |
...
**S1a + S1b + S2 + S3a + S3b + S4 + S5b + S5c are all shipped and archived** — the whole
variety arc is complete bar the no-build S5a: the five readouts (usage · opener · degrade ·
occupancy · scoring) over the frozen gauntlet, the committed, drift-guarded variety board
`docs/variety.md` (S5b — plan #291 / slice #292 / archived #293; generated by
`npm run gen:variety`, pinned by a `toBe generateVariety()` drift test), and the public
`/variety` web surface (S5c — Slice 1 page #295 / Slice 2 discoverability #296; a static,
no-JS `/spec-guide` clone byte-derived at build from `generateVariety()`, discoverable via
sitemap + llms.txt + an Arsenal link, NOT top-nav; plan archived
`docs/archive/variety-telemetry-s5c.md`).

**S5a** (external submission corpus — **no build**, just the S1b `-- <path…>` override
pointed at a submissions dir) remains the only post-launch item, its sole dependency being
real submissions.
