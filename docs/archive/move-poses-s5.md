...
point is a joint the bend rule currently _computes_. S5 promotes the mid-joint to a first-class driven
...
**Value**: _Actor_ — the spectator at `/watch` (and the developer in `/dojo`). _Outcome_ — a committed
...
- **`scene.ts`**:
  - A driven **mid-joint** is `elbow*` / `knee*`. Gate **both `lean` and `step` to 0** for a driven
    ...
    `ARM_BONE`** ≈ 31px), `empi`'s target (~24–30px) is _already_ within reach, so `lean` computes to 0
    ...
    the _shoulder_ is conceptually wrong even when it numerically lands 0). So do not expect the "no
...
drivenPoint`. This realises M13(b)'s "skip the driven mid-joint" _observably_ — the derived value is
...
- **RED**: with `a` committing `empi` (active phase, in range), assert on `a`'s scaled pose:
  - `elbowR` sits at the solved reach-to-target `{ x: reachTargetX, y: bandHeight }` (computed from the
    ...
    is _not_ the `deriveBend(shoulderR, handR)` bisector the old path would produce.
  ...
  - `footL`, `footR`, `handL` unchanged from stance; `head.x` / `shoulder.x` at stance x (**no lean** —
    guards 1 + 2 + the M13f root-held rule; a _standing_ guard, numerically already 0 at `empi`'s reach,
    not the RED driver).
...
driven }`, so the _front hand_ is driven and `elbowR` is only the derived bisector — the "elbowR at
...
**Value**: _Actor_ — the spectator / developer. _Outcome_ — a committed `hiza-geri` renders as a knee
