...
**Status**: Complete — all 3 slices shipped, all 8 ACs met. Archive this file under `docs/archive/`.
...
- [x] AC1 — the nav's **Fights** item navigates to `/watch` (a real `<a href>`, not a scroll anchor) → **Slice 2** (#424)
- [x] AC2 — on `/watch`, the nav's **Fights** item carries `aria-current="page"` → **Slice 2** (#424)
- [x] AC3 — the **prerendered** home HTML contains a link to `/watch` and no `aria-disabled` element → **Slice 2** (#424)
- [x] AC4 — the home Fights section still carries `id="fights"`, so `/#fights` resolves → **Slice 2** (#424)
- [x] AC5 — the string "in development" appears nowhere on the home page → **Slice 2** (#424)
- [x] AC6 — `sitemap.xml` lists `https://modelkombat.club/watch`, and no `/watch/{id}` → **Slice 3**
- [x] AC7 — `llms.txt` documents `GET /replay` (both public URL forms + size caveat) and `/watch` → **Slice 3**
