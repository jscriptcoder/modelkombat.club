...
- ✅ **Slice 4 — `mae-geri` front kick** — MERGED (PR #71). `MoveId += "mae-geri"`; `MOVES` +
  ...
  rejects **both** high and low) + first **2-point (_waza-ari_)** strike (`score 2` flows
...
| 6 ✅ | **Bot can chain techniques via cross-move cancels** [MERGED #73]                         | Combos: a hit-confirmed technique cancels into a _different_ one (the rekka routes) — within-exchange escalation across the roster                   | Canonical `cancelInto` routes between distinct moves (e.g. jab→reverse→roundhouse); a `runFight` proof that cross-move cancel resolves and preserves the **no-feint / connect-required** property (whiff/parry never opens it)                                                                                                                                                                       | The exact tuned route table (canonical-content, lands here or in slice 7)                             | Given a jab that connects with jab.cancelInto⊇[reverse], When the bot returns `gyaku-zuki` within `cancelWindow`, Then recovery is interrupted into it; When the jab whiffed, Then the cancel is ignored | Shippable                                                  |
...
strike 240k`); per-move `staminaCost` (cheap jab → expensive roundhouse, preserving the gas band
   ...
   in S6). Re-prove every number by a _relationship_ test in `rules.test.ts` (master inequalities +
