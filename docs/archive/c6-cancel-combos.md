...
- **Design source of truth:** `docs/DESIGN.md` §3 (combos — on-contact cancels only) and
  ...
  _documented, deferred insertion point_ — C6 is the slice that builds it. §11.3 makes the
...
- **The §11 compute-then-apply union is already live (built in C5).** `computeStrike` (pure,
  ...
  self-targeted effect** (it opens a window on the _attacker_), so C6 slots into the existing
...
- **No real frame table yet** (`MoveId = "strike"` only; concrete moves live in test mocks).
  ...
  distinct cancel routes is a later, additive slice). The `cancelInto` _route list_ is still
...
- **The cancelable window** is tied to the connecting move and follows the **`counterRemaining`
  ...
  attacker; a fired cancel starts a fresh move (window resets to closed until _that_ move
...
- **Intake gets the one surgical exception to commitment:** today `intake` honours an action
  ...
  (`canAct = 0`) fighter whose move is **cancelable** _and_ whose returned `attack` is a **legal
...
opponent _already perceived_ the hit connect).
...
is cancelable _and_ its returned `attack` is in `rules.moves[move].cancelInto` → replaces the
...
(Slice 2), the `self.cancelWindow` read (Slice 3 — Slice 1's bot cancels _blindly_, spamming
...
follow-up's `attackTicks[1]` is the _cancelled_ (early) tick, not the full-recovery tick.
...
- `intake`: honour an `attack` from a **committed** fighter iff its move is cancelable _and_ the
...
remains "no score, no stun" _except_ it now enables cancel); the `self.cancelWindow` read.
...
2. **Parry still does not.** A **fresh** matching guard (PARRY) still leaves the move
   uncancelable (Slice 1's AC-3 still green) — the trigger is hit _or block_, never parry.
...
1. **Cancel demonstration = self-cancel (`strike → strike`).** Recommended — keeps `MoveId`
   ...
   later additive slice. The route list is still a live gate (empty ⇒ no cancel). _Alternative:_
...
2. **Slice 3 read surface = `self.cancelWindow`** (numeric ticks-remaining, mirrors
   `self.counterWindow`). Recommended for consistency. _Alternatives the docs also name:_
...
3. **No cancel-chain cap in C6.** A self-cancel against a passive in-range opponent could chain
   each tick the window allows (within-exchange escalation is _intended_). Unbounded loops are
   bounded later by _yame_ (exchange reset) + stamina, not by C6. Tests bound chains with
...
_Delete this file when C6 is complete. If `plans/` is empty, delete the directory._
