...
**Why this first:** it burns down the only genuinely *novel* risk in the arc — deterministic
...
- **True-permanent replay retention** — per-record keyed storage (replayId → record) + a list
  ...
  hard promise.
- **Richer home "Arena / House Fighters" section** — S2 just *removes* the Gauntlet section; a
...
- **S2 API↔web coupling.** The `/ring` UI keys on `body.cleared`; if the reshaped API ships
  before the UI it shows "Didn't clear the gauntlet" for everything. Land them together (or a
  shape-tolerant UI first) — a planning/staging concern, not a component split. Do **not** split
  S2 into "API story" + "web story."
...
Load `planning` for **S1 — Fresh seeded v20 season** to turn it into PR-sized implementation
slices (each running the full `tdd` → `testing` → `mutation-testing` → `refactoring` cycle
before code). Optionally run `find-gaps` on this split first to harden the acceptance examples.
