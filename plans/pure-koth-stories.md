...
- **S2 — Drop the gauntlet: ✅ SHIPPED & CLOSED** (2026-07-23; PRs #401–#405, `main`@`8578a20`). Plan
  archived → `docs/archive/pure-koth-s2.md`. The gauntlet gate is gone: a bot fights the sitting
  champions directly (S2.1 #402, −876 lines), `/spec` + `llms.txt` describe the climb naming no
  fighters (S2.2 #403, D16), and the arena-mirror guard is model-agnostic (S2.3 #404, D17).
- **S3 — Watch every competing fight:** next (the last slice of the arc).

## Next Step

Load `planning` for **S3 — Watch every competing fight** — the final slice of the arc — to turn it
into PR-sized implementation slices (each running the full `tdd` → `testing` → `mutation-testing` →
`refactoring` cycle before code). `grill-me` + `story-splitting` are already done for the whole arc
(see `pure-koth-decisions.md` D1–D17 + the split above), so S3 goes straight to planning; optionally
run `find-gaps` on the S3 row first to harden its acceptance examples and settle the open planning
questions — the per-bout content-hash id scheme (D14) and the **Upstash `LRANGE` ceiling prereq**
before raising `DEFAULT_ARCHIVE_LIMIT` 50→200 (confirm a full-archive reply fits the plan's response
ceiling + acceptable cold-read latency; fall back to the largest safe cap if not).
