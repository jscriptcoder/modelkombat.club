...
- **Seed default.** An empty store must resolve to `SEED_ARENA` for BOTH the read path
  (`handle-king`) and the compete path (`handle-fight`). `handle-replay` reads the *archive*
...
- [ ] On an empty arena, `GET /king` returns the three strongest House bots ranked King/#2/#3 by
      a deterministic build-time round-robin, each credited `handle: "House"`.
...
store), the same pattern the existing handler tests use.
...
- Given an empty store, `GET /king` returns exactly three entries, ranked King/#2/#3 in the
  deterministic seed order, each with `handle: "House"` and identity-only fields (never a doc).
...
- Given a non-empty store, `GET /king` is byte-unchanged (the resolver passes the real arena
  through) — no regression to the current `/king`.
**RED**: A `handle-king` test with an injected empty store expecting the three ranked House
rows; and a `seed-arena` test pinning the constant against the recomputed round-robin ordering
(reusing `winRateVsRoster` from `gauntlet-calibration.test.ts`).
**GREEN**: Add `SEED_ARENA` (+ a `buildSeedArena(bots)` hydrator from the loaded gauntlet docs)
and `readArenaOrSeed`; wire `handleKing` to it.
...
- **House `model`**: the archetypes have no LLM model ⇒ `model` is null/absent → no `ModelLogo`;
  `/king` + `/ring` render the `[House]` handle credit only.
- **Web**: no `/king` or `/ring` layout change is needed in S1 — the existing board renders the
  seeded members and `handle: "House"` as ordinary champion rows. (Any "House badge" styling is a
  cosmetic follow-up, not required for S1.)
