...
(hand-written) *and* "regenerates deterministically" (generated). A grill-me pass
...
2. **Content → REUSE the CLI text, fenced.** The board body is the *exact*
   ...
   scaffold. Near-zero new render code; the board is byte-identical to what
   `npm run telemetry` prints. (Rejected: a second set of native-markdown-table
...
4. **§P7 verdict → MINIMAL scaffold + a STATIC note.** The scaffold adds a title
   ...
   *static* one-line §P7 orientation note (soft targets `usage ≤ 35%`,
   `opener win ≤ 60%`; "scan for ⚠ below"). The actual pass/fail lives *only* in the
...
---
*Archive (don't delete) when complete, per `docs/archive/README.md` + the
`archive-plans-not-delete` rule.*
