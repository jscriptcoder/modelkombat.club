...
- [ ] **AC-6 (TCB allowlist):** `validate` **accepts** a bot whose action is
  ...
  `band:"low"` (band-legality is a *runtime* concern; the validator only checks the move id +
...
*between two strikes* (poke with the fast short jab vs commit the longer reverse).
...
If `plans/` is then empty, delete the directory.*
