...
**`/dojo` pose lab** — the two-figure tuning harness.
...
not spectator — stated explicitly. If you'd rather lead with spectator value, **heads (story 2)**
...
(likely: walking-skeleton route+render → pose controls → gap slider+presets). Each planned
slice repeats the full cycle — `tdd` · `testing` · `mutation-testing` · `refactoring`,
RED→GREEN→MUTATE→KILL→REFACTOR — before the next begins. Optionally run a quick `find-gaps` pass
