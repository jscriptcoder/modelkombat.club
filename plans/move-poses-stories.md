...
Load `planning` for **S0 + S1**. They are small enough to plan together — S0 is a single
additive engine change and S1 is its first consumer — and planning them as a pair keeps the
byte-identical gate adjacent to the change that proves the field is actually usable.
