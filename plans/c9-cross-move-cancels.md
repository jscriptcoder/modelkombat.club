...
*different* one (the rekka combo, e.g. jab → reverse → roundhouse) — and that the **no-feint /
...
- `intake` cancel path (sim.ts:364-378) reads `rules.moves[action.move]` generically and checks
  ...
  `MoveId[]`, so it lists *distinct* moves.
...
assert the *intended* cross-move combo behavior and lock it against regression. They pass green
