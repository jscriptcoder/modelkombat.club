...
**Status**: Active — **plan approved 2026-07-10 (defer all `/king`/podium edits to S3)**; ready to implement Slice 1
...
1. ✅ **Defer *all* `handle-king`/podium edits to S3.** The story doc lists "`/king` reads arena[0]" under
   ...
   suite passes unchanged" is *literally* true.
...
### Slice 1: The store persists an atomically-CAS'd top-N arena record (fake + Upstash + contract), at N=1
...
### Slice 2: `POST /fight` decides crowning through the arena record at N=1 (behavior-identical)
...
---
*Delete this file when S1 is complete (or fold remaining S-slices into their own plans). If `plans/` is empty, delete the directory.*
