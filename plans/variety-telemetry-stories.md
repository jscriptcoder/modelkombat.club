...
**S3b, S4, S5b, and S5c each resolved via a grill-me pass** (2026-07-13 → S3b 9 examples,
S3b-1…S3b-9; S4 11 examples, S4-1…S4-11; S5b board design, decision #11; S5c `/variety`
page design, decision #12). Decisions locked in the sibling scoping doc
`variety-telemetry-harness.md` (§Resolved decisions #1–#12). Feeds `planning`, one
...
complete. **S5b** (the committed, drift-guarded variety board `docs/variety.md`) is
**shipped + archived** (plan #291 / slice #292, plan archived at
`docs/archive/variety-telemetry-s5b.md` via #293) — the committed `docs/variety.md` is
generated by `npm run gen:variety` and pinned by a `toBe generateVariety()` drift test.
**The entire internal-CLI variety arc (S1a–S5b) is now COMPLETE**; only post-launch
**S5a** (submission corpus — no build) and **S5c** (public web meta-report surface)
remain.
...
| **S5b** | "Can the team review a variety snapshot?" | **✅ SHIPPED + ARCHIVED** (plan #291 / slice #292 / archived #293): evergreen committed `docs/variety.md`, **generated + byte-match-drift-tested** (the `docs/spec.md` trio, NOT the hand-written gauntlet boards), body = verbatim `npm run telemetry` output fenced in a thin scaffold | — | drift test asserts committed `docs/variety.md` `toBe` `generateVariety()`; `npm run gen:variety` reproduces it byte-for-byte | repo artifact; **✅ shipped + live** |
| **S5c** | "Can the community see the meta?" | **GRILLED + PLANNED** (decision #12, `plans/variety-telemetry-s5c.md`): a static prerendered `/variety` page cloning `/spec-guide` — renders the `docs/variety.md` board via `marked`, `generateVariety()` regenerated at build, no client JS; discoverable via sitemap + llms.txt + an Arsenal link (NOT top-nav) | — | `/variety` serves the rendered board as no-JS HTML; sitemap + llms.txt list it | public web; **shippable now** |
...
**S1a + S1b + S2 + S3a + S3b + S4 + S5b are all shipped and archived** — the entire
internal-CLI variety arc is complete: the five readouts (usage · opener · degrade ·
occupancy · scoring) over the frozen gauntlet, plus the committed, drift-guarded variety
board `docs/variety.md` (S5b — plan #291 / slice #292 / archived #293; generated by
`npm run gen:variety`, pinned by a `toBe generateVariety()` drift test).

Two variety stories remain. **S5c** (the public web meta-report surface) is now
**grilled + planned** (plan at `plans/variety-telemetry-s5c.md`, decision #12): a static
prerendered `/variety` page cloning `/spec-guide` — it renders the `docs/variety.md`
board via `marked` with `generateVariety()` regenerated at build (no client JS), and is
discoverable via sitemap + llms.txt + an Arsenal link (not top-nav). Its two TDD slices
(the page end-to-end, then discoverability) are the next build once this `docs(plan)` PR
merges (the "no plan on main" lesson) — `planning` → TDD, PR per slice. **S5a** (external
submission corpus — **no build**, just the S1b `-- <path…>` override pointed at a
submissions dir) remains the only post-launch item, its sole dependency being real
submissions.
