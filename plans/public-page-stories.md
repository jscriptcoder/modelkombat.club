...
collision, rewrite precedence, build command) — *while already delivering value* (a real
...
- **AC-K2 (empty throne):** Given no King yet for the current version (the likely launch
  ...
  `{ current: null, recent: [] }` — a *success* response, not an error. The King section
...
- **AC-L3 (a11y):** each mark has an accessible label naming the model
  (`aria-label="authored by Claude"` / alt text) — the logo is never the *only* signal;
...
- **AC-R3 (touch):** the fights "in development" state is conveyed by a **visible
  label/badge on the disabled control**; the hover tooltip is an *enhancement only* (touch
...
- **Persist title fights** (challenger/incumbent identity, outcome, winRate, **seeds**,
  transport-layer timestamp) → makes the fights list *real* and captures the seeds that
